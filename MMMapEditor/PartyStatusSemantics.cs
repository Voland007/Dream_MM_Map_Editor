...
namespace MMMapEditor
{
    public static class PartyStatusSemantics
    {
        ...
        public const byte NonDeadTrackedMask = DiseasedMask | PoisonedMask | ParalyzedMask | UnconsciousMask;
        public const byte TrackedMask = NonDeadTrackedMask | DeadMask;
        ...
        public static List<string> GetStatusNamesForExactValue(byte value)
        {
            ...
            if (IsDeadStatusValue(value))
                return new List<string> { "DEAD" };

            var result = new List<string>();
            AddNonDeadTrackedStatusNames(result, value);
            return result;
        }
        ...
        public static bool IsDeadStatusValue(byte value)
        {
            return value > DeadMask && value != EradicatedValue;
        }

        public static bool HasTrackedStatuses(byte value)
        {
            return value == GoodValue ||
                   value == EradicatedValue ||
                   (value & NonDeadTrackedMask) != 0 ||
                   IsDeadStatusValue(value);
        }

        private static void AddNonDeadTrackedStatusNames(List<string> result, byte value)
        {
